...
# 定义batch_size 训练轮数 max_steps 下载cifar-10的默认路径
max_steps = 3000
batch_size = 128
data_dir = '/tmp/cifar10_data/cifar-10-batches-bin'

# 根据l2正则化loss创建卷积核参数   使得特征的权重称为损失函数的一部分
...
label_holder = tf.placeholder(tf.int32, [batch_size])   # 注意 这里的数据尺寸中的第一个值是样本条数，必须预先设定  因为batch_size在之后的网络结构设定中会用到

'''
第一层卷积层
'''
# 卷积核参数初始化， 且第一层卷积不进行正则化
weight1 = variable_with_weight_loss(shape=[5, 5, 3, 64], stddev=5e-2, w1=0.0)
kernel1 = tf.nn.conv2d(image_holder, weight1, [1, 1, 1, 1], padding='SAME')
bias1 = tf.Variable(tf.constant(0.0, shape=[64]))
conv1 = tf.nn.relu(tf.nn.bias_add(kernel1, bias1))
pool1 = tf.nn.max_pool(conv1, ksize=[1, 3, 3, 1], strides=[1, 2, 2, 1],
                       padding='SAME') # 池化为3*3 步长2*2
## 加入了LRN层 侧抑制，强特征更强，反馈小的特征更小
